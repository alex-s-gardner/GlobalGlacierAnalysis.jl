...
begin
    ...
    out_file = "/mnt/bylot-r3/data/gemb/raw/$file_uniqueid.jld2"
    ...
    Δd =  30;
    date_range = DateTime(1990):Day(Δd):now();
...
if .!(isfile(out_file)) || force_remake   
    gemb_files = allfiles(gemb_folder; subfolders=falsefn_endswith=".mat",fn_contains=file_uniqueid)
    expected_num_files = length(elevation_delta) * length(precipitation_scale)
    ...
    save(out_file, Dict("gemb" => gemb));
else
    gemb = load(out_file, "gemb")
end