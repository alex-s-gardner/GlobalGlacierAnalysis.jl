    ...
    project_id = "v01"
    param= Altim.binned_filled_fileparts(synthesized_gemb_fit)
     binned_folder =  "/mnt/bylot-r3/data/binned_unfiltered/2deg"
    fig_folder = joinpath(binned_folder , "figures")
        !isdir(fig_folder) && mkdir(fig_folder)

        param_filling = Altim.binned_filling_parameters[param.fill_param]
        ...
        # paths to files
        binned_file = Altim.binned_filepath(; binned_folder, param.surface_mask, dem_id = param.dem, param.binning_method, project_id, param.curvature_correct)
        binned_file_land = Altim.binned_filepath(; binned_folder, surface_mask = :land, dem_id = param.dem, param.binning_method, project_id, param.curvature_correct)
        binned_filled_file, figure_suffix = Altim.binned_filled_filepath(; binned_folder, param.surface_mask, dem_id = param.dem, param.binning_method, project_id, param.curvature_correct, param.amplitude_correct, paramater_set = param.fill_param)
