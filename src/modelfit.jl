...
function ts_fit(
    ...
    if (length(x) >= p.count_min) && (Altim.range(t) >= p.t_min_range)
        ...
        # fit offset and trend using robust methods
    ...
    if isnothing(df)
        ...
        #for i in eachindex(griddef.x_node_center)
            ...
            for j in eachindex(griddef.y_node_center)
                ...
                # should we continue ?
                Altim.range(decyear0[start:stop]) >= p.t_min_range || continue

                x_cord[i, j], y_cord[i, j], height[i, j], count[i, j], coefficeints[i, j, :], mad_out[i, j] =
                    ts_fit(x0[start:stop], y0[start:stop], h0[start:stop], decyear0[start:stop], dh0[start:stop]; w=w0[start:stop], p = p);
            end
        end
        t4 = time()
