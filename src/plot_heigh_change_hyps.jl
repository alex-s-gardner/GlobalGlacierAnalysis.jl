begin
    ...
    if amplitude_correct
        figure_suffix = "$(figure_suffix)_ac"
    end
...
if true
    ...
    for rgi in lookup(dm_reg_masked, :rgi)
        ...
        dFac_mean = (dmFac_mean .- dv_mean)
        dFac_mean = dFac_mean[1:end-1] .- dFac_mean[2:end]

        histogram(dFac_mean, bins=0:100:2000)
        ...
        valid = .!isnan.(dm_reg_masked[:, At(rgi), :])
        dvX .+= vec(sum(dm850 .* valid.data, dims=1) ./ sum(valid.data, dims=1))
        dmX .+= vec(sum(dmFac .* valid.data, dims=1) ./ sum(valid.data, dims=1))
        nobsX .+= nobs_reg_masked[:, At(rgi), :]
    end
...
begin
    ...
    for rgi in vcat(collect(dims(dm_reg_masked, :rgi)))
        ...
        fname = joinpath(fig_folder, "$(rgi)_altim_grace.png")
        save(fname, f)
        showplots && display(f)
    end
    ...
    fname = joinpath(fig_folder, "global_altim_grace.png")
    save(fname, f)
    showplots && display(f)
...
begin
    ...
    fname = joinpath(fig_folder, "global_altim_grace_zemp.png")
    save(fname, f)
    showplots && display(f)
end
...
begin
    ...
    fname = joinpath(fig_folder, "global_altim_zemp.png")
    save(fname, f)
    showplots && display(f)
end
...
begin
    ...
    fname = joinpath(fig_folder, "global_altim_marzeion2012.png")
    save(fname, f)
    showplots && display(f)
end
...
begin
    ...
    fname = joinpath(fig_folder, "global_altim_marzeion2020.png")
    save(fname, f)
    showplots && display(f)
end
...
begin
    ...
    fname = joinpath(fig_folder, "global_altim_ipccar6_fig9_21.png")
    save(fname, f)
    showplots && display(f)
end
...
begin
    ...
    fname = joinpath(fig_folder, "global_altim_ipccar6_fig9_21_start2007.png")
    save(fname, f)
    showplots && display(f)
end
