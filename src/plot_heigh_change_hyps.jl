...
# create a plot with all permutations & combinations
...
begin
    ...
    for hugonnet_filtered_flag in hugonnet_filtered_flags
        hugonnet_filtered_flag = true

        binned_folder = analysis_paths(; geotile_width).binned

        if !hugonnet_filtered_flag
            #-------------------------------------
            binned_folder = replace(binned_folder, "binned" => "binned_unfiltered")
            #-------------------------------------
        end

        # load and combine ALL model runs
        if hugonnet_filtered_flag
            dvdm_reg_files = hcat(
                Altim.dvdm_reg_filepath(binned_folder, "glacier", project_id, 1),
                Altim.dvdm_reg_filepath(binned_folder, "glacier", project_id, 2),
                Altim.dvdm_reg_filepath(binned_folder, "glacier_rgi7", project_id, 1),
            )
        else
            dvdm_reg_files = hcat(
                Altim.dvdm_reg_filepath(binned_folder, "glacier", project_id, 1),
        ...
        for dvdm_reg_file in dvdm_reg_files
            dv1 = load(dvdm_reg_file, "dv");
            dm1 = load(dvdm_reg_file, "dm");
            nobs01 = load(dvdm_reg_file, "nobs");
            df1 = load(dvdm_reg_file, "df");

            if !hugonnet_filtered_flag
                dv1 = Dict(keys(dv1) .* "_unfilt"  .=> values(dv1))
                dm1 = Dict(keys(dm1) .* "_unfilt"  .=> values(dm1)) 
                nobs01 = Dict(keys(nobs01) .* "_unfilt"  .=> values(nobs01))
                df1[!, "hugonnet_filter"] .= false;
            else
                df1[!, "hugonnet_filter"] .= true;
            end

            dv = push!(dv, dv1);
            dm = push!(dm, dm1);
            nobs0 = push!(nobs0, nobs01);
            df = push!(df, df1);
        end

        println(dvdm_reg_files)
    end
    ...
    t = collect(dims(dm[first(keys(dm))], :date))
...
reg = vcat(collect(lookup(dm[first(keys(dm))], :rgi)), "global");
for rgi in ["rgi19"]
    ...
    index = (df.rgi .== rgi) .& (df.amplitude_correct) # .& (df.binning_method .== "meanmadnorm3") .& df.hugonnet_filter 
    f = Altim.plot_dm_permutations(t, df[index, :], title; colors=ColorSchemes.colorschemes[:vanimo100], heightanomaly_flag=true )

    fname = joinpath(fig_folder, "all_model_permutations_$(rgi).png")
    save(fname, f)
    showplots && display(f)
end

for hugonnet_filtered_flag in hugonnet_filtered_flags
    #hugonnet_filtered_flag = false
    ...
    dvdm_reg_file = Altim.dvdm_reg_filepath(binned_folder, "glacier", project_id, 1)
    ...
    for surface_mask in surface_masks
        ...
        if (surface_mask == :glacier) && hugonnet_filtered_flag
            paramater_sets = [1, 2]
        elseif (surface_mask == :land)
            paramater_sets = [1, 2]
        else
            paramater_sets = [1]
        end
    ...
    for paramater_set in paramater_sets
        #paramater_set = first(paramater_sets)
            ...
            begin
                # path to file
                dvdm_reg_file = Altim.dvdm_reg_filepath(binned_folder, surface_mask, project_id, paramater_set)
            ...
            if true
                ...
                #rgi = "rgi19"
                    ...
                    f = Altim.plot_dvdm(dv_reg_masked[:, At(rgi), :], dm_reg_masked[:, At(rgi), :], nobs_reg_masked[:, At(rgi), :]; title, fontsize, colors, date_intercept, area=area[At(rgi)], area_average_flag=false)

                    fname = joinpath(fig_folder, "$(figure_suffix)_$(rgi).png")
                    save(fname, f)

                    f = Altim.plot_dvdm(dv_reg_masked[:, At(rgi), :], dm_reg_masked[:, At(rgi), :], nobs_reg_masked[:, At(rgi), :]; title, fontsize, colors, date_intercept, area=area[At(rgi)], area_average_flag=true)
                ...
                f = Altim.plot_dvdm(dvX, dmX, nobsX; title, fontsize, colors, area=sum(area.data))

                fname = joinpath(fig_folder, "$(figure_suffix)_global.png")
                save(fname, f)
                showplots && display(f)

                f = Altim.plot_dvdm(dvX, dmX, nobsX; title, fontsize, colors, area=sum(area.data), area_average_flag=true)
                fname = joinpath(fig_folder, "$(figure_suffix)_global_dh.png")
                save(fname, f)
                ...
                for rgi in vcat(collect(lookup(dm_reg_masked, :rgi)), "global")
                    ...
                    f = Altim.plot_dm_permutations(t, dm, title; colors=ColorSchemes.colorschemes[:tab20c])
            ...
            if true
                ...
                for rgi in lookup(dm_reg_masked, :rgi)
                    ...
                    dv_mean, dv_fit = Altim.region_fit(dv, date_intercept)
                    dmFac_mean, dm_fit = Altim.region_fit(dmFac, date_intercept)
                    δ_effective = round(Int, dm_fit.param[2]./dv_fit.param[2] * 1000)

                    # deteremine monthly fac equivelent
                    dv_time_averaged = median.(DimensionalData.groupby(dv_mean, :date=>year))
                    dmFac_time_averaged = median.(DimensionalData.groupby(dmFac_mean, :date=>year))
            
                    dmFac_time_averaged = dmFac_time_averaged[2:end] .- dmFac_time_averaged[1:end-1] 
                    dv_time_averaged = dv_time_averaged[2:end] .- dv_time_averaged[1:end-1] 

                    δ_effective_timeseries = dmFac_time_averaged ./ dv_time_averaged * 1000
                    
                    # check effective 
                    if false
                        p = plot(δ_effective_timeseries)
                        #ylims!(0,2000)
                        p
                    end
                    ...
                    f = Altim.plot_dvdm(dm850, dmFac, nobs_reg_masked[:, At(rgi), :]; title, fontsize, colors, date_intercept, area=area[At(rgi)], dmdm_flag=true, δ_effective, δ_effective_timeseries)
                ...
                dv_mean, dv_fit = Altim.region_fit(dvX, date_intercept)
                dmFac_mean, dm_fit = Altim.region_fit(dmFacX, date_intercept)
                δ_effective = round(Int, dm_fit.param[2] ./ dv_fit.param[2] * 1000)
                ...
                f = Altim.plot_dvdm(dm850X, dmFacX, nobsX; title, fontsize, colors, area=sum(area.data), dmdm_flag=true, δ_effective, δ_effective_timeseries)
            ...
            begin
                ...
                for rgi in vcat(collect(dims(dm_reg_masked, :rgi)))
                    ...
                    area0 = area[At(rgi)]
                    f = Altim.plot_dm([grace, altim]; title, fontsize=18, date_intercept=2012, area=sum(area[At(rgi)]), xlim, date_alignall=(2003, 2004), colors=ColorSchemes.colorschemes[:Set1_9], scale_color=false, stats_in_label=true,)
                ...
                f = Altim.plot_dm([grace, altim]; title, fontsize=18, date_intercept=2012, xlim, date_alignall=(2003, 2004), colors=ColorSchemes.colorschemes[:Set1_9], scale_color=false, stats_in_label=true)
            ...
            begin
                ...
                for rgi in vcat(collect(dims(dm_reg_masked, :rgi)))
                    ...
                    f = Altim.plot_dm([zemp, altim]; title, fontsize=18, date_intercept=2012, xlim, date_alignall=(2003, 2004), colors=ColorSchemes.colorschemes[:Set1_9], scale_color=false, stats_in_label=true,)
                ...
                f = Altim.plot_dm([grace, altim, zemp]; title, fontsize=18, date_intercept=2012, area=sum(area[At(rgi)]), xlim, date_alignall=(2003, 2004), colors=ColorSchemes.colorschemes[:Set1_9], scale_color=false, stats_in_label=true)
            ...
            begin
                ...
                f = Altim.plot_dm([altim, zemp]; title, fontsize=18, date_intercept=2012, area=sum(area[At(rgi)]), xlim, date_alignall=(2000, 2002), colors=ColorSchemes.colorschemes[:Set1_9], scale_color=false, stats_in_label=true)
            ...
            begin
                ...
                marzeion12 = Altim.read_marzeion2012()
                ...
                f = Altim.plot_dm(vcat(m12, [altim]); title, fontsize=18, date_intercept=2012, xlim, stats_in_label=true)
            ...
            begin
                ...
                marzeion20 = Altim.read_marzeion2020()
                ...
                f = Altim.plot_dm(vcat(m20, [altim]); title, fontsize=18, date_intercept=2012, xlim, date_alignall=(2005, 2010), stats_in_label=true)
            ...
            begin
                ...
                f = Altim.plot_dm(vcat(AR6, [altim]); title, fontsize=18, date_intercept=2020, xlim, date_alignall=(2014.5, 2015.5), stats_in_label=true)
            ...
            begin
                start2007 = true;
                title = "Global: IPCC AR6"
                IPCCAR6 = Altim.read_ipccar6(; start2007)
                scenarios = dims(IPCCAR6.dm_gt, :scenario)
                ...
                f = Altim.plot_dm(vcat(AR6, [altim]); title, fontsize=18, date_intercept=2020, xlim, date_alignall=(2006.5, 2007.5), stats_in_label=true)
