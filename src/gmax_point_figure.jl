...
begin #[50s]
    ...
    glacier_flux = Altim.nc2dd(glacier_flux["runoff"]) / (1000 * u"kg/m^3")
    land_flux = Altim.nc2dd(land_flux["flux"])
    seperate_out_snow && (snow_flux = Altim.nc2dd(snow_flux["flux"]))
...
location = locations[1]
    ...
    lines!(ax2, [Altim.decimalyear(dates4plot[1]), Altim.decimalyear(dates4plot[end])], [gmax_min, gmax_min]; color = (:gray, 0.5), linestyle = :dash)
    lines!(ax2, [Altim.decimalyear(dates4plot[1]), Altim.decimalyear(dates4plot[end])], [gmax_max, gmax_max]; color=(:gray, 0.5), linestyle=:dash)
    lines!(ax2, [Altim.decimalyear(dates4plot[1]), Altim.decimalyear(dates4plot[end])], [gmax, gmax]; color=(:black, 1))
    ax2.ylabel = "glacier fraction [%]"

    xlims!(ax2, minimum(xticks), maximum(xticks))
    text!(Altim.decimalyear(dates4plot[end]), mean((gmax_max, gmax)), text="gmax = $gmax [$(Dates.monthname(gmax_month))] ", align=[:right, :center], color=(:black, 0.8), overdraw=true)
    display(fig)
